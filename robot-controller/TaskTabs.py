...
class General (QtGui.QWidget):
    def __init__(self, nao, parent):
        ...
        self.intro = SpeechButton("Intro", "Thanks for coming in. Please remember that you can leave at any time. If you have any specific questions about the tasks, please hold them until after the experiment is complete; I will be happy to answer any of your questions afterwards. If you are uncomfortable with this, please rememeber that you can leave at any time. The cash honorarium is yours to keep even if you choose not to continue to the end.", nao)
        self.demographics = SpeechButton("Demographics", "Before wee begin, please open the blue folder in front of you. Inside, you will find a demographics questionnaire. Please fill it out and let me know when you are finished.", nao)
...
class Tedium (QtGui.QWidget):
    ...
    def __init__(self, nao, parent):
        ...
        self.explanation = SpeechButton("Explanation", "Currently, the process of renaming files in a graphical user interface is not very efficient. We are trying to make this more efficient for users, so we are collecting data on how people do this and are feeding the data into a machine learning algorithm. This laptop will record your exact mouse movements, keyboard strokes, and precise timing to create a model of the work you need to do to rename files. How quickly you can do this is not important, so do not worry about performance. Machine learning algorithms require a great deal of data, hence the need to do this repeatedly." , nao) 
        ...
        self.change = SpeechButton("Change", "Change the last part of the file name that says: dot, jay pee gee, to dot, pee n gee. For example: A file called robot, dot, jay pee g, should be renamed to robot, dot, pee n gee.", nao)
        ...
        self.good = SpeechButton("Good", "Good. Please continue to do this for each file in the folder, and let me know when you are done.", nao)
        self.shortcuts = SpeechButton("Shortcuts.", "Because we are measuring precise keystrokes and mouse movements, please do not use shortcuts such as copy and paste, as this may confuse the machine learning system.", nao)
...
class MentalChallenge (QtGui.QWidget):
    def __init__(self, nao, parent):
        ...
        self.explanation = SpeechButton("Explanation", "Currently, computers are not very good at solving Rubik's Cubes. Computers can brute force the problem or use simple tricks, but humans are able to solve the problem much more organically. Because we can not represent the complex neurological processes of a human brain in a computer, we can use some algorithms to learn from peoples' actions. The web cam on the laptop is going to record your movements and learn from your experience. It does not matter whether or not you can solve the cube because all data, successes and failures, is useful to the algorithm. Please do not feel pressured: most people are not able to solve these puzzles. Approach this as a game and simply enjoy the puzzle, but do try your best to solve it." , nao) 
        self.camera = SpeechButton("Turn on camera", "I will now turn on the camera.", nao)
        self.turn = SpeechButton("Turn towards camera", "Please turn towards the blue light, and keep your hands in the general direction of the light.", nao)
        ...
        self.positionHands = SpeechButton("Position hands", "Please position your hands more in the direction of the camera.", nao)
        self.solve = SpeechButton("Solve cube.", "Now, please try solving the cube.", nao)
        self.data = SpeechButton("Data", "It is important to get as much data as we can for the machine learning system. We will do this as long as you can, but, let me know when you're done.", nao) 
        self.mixUp = SpeechButton("Mix up and try again.", "Good. Now, please mix up the cube and solve it again. Continue to do so each time you solve it.", nao)
        self.done = SpeechButton("Done", "Thank you, that should be good for now.", nao)
